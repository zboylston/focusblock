...
- Focus-card scratchpad: while in focus mode with an active task name, a "Notes" area on the timer card is a Notion-style live-markdown editor (TipTap + tiptap-markdown) for that task's note (the same per-group `description` shown in the timeline). Type `# `, `- `, `1. `, `- [ ] `, `> `, ```` ``` ````, `**bold**` etc. and it formats inline; stored/serialized as markdown so the timeline's plain-text preview still works. Looked up/saved by name via `GET/PUT /tasks/note`, which resolve the MOST RECENT group for that name (any day) so a carried-over task still surfaces and edits its existing note; only when no group exists at all does saving materialize a single open block (today) to hold it. Saves on blur plus a 1.5s debounced autosave. The card is dynamic: it widens into scratchpad mode (timeline shrinks but stays readable) — auto-expands when a focus timer starts and snaps back on reset/complete/switch-to-break, plus a manual expand/collapse toggle next to the "Notes" label
...
- Inline rename: hovering a Timeline task group reveals a pencil; clicking swaps the title for an input (save on Enter/blur, Escape cancels) that renames every block of that `(name, date)` group atomically via `POST /tasks/rename`. Rejects with a toast if another group already uses that name on the same day (409)
...
- Any route that INSERTS or moves rows within a name's group — appends (`complete-focus` append branch, `add-block`, the note-create path), `POST /tasks` create, and `POST /tasks/rename` — must run inside a transaction holding `pg_advisory_xact_lock(hashtext(name))`. There is no DB unique constraint on `(name, date, chunkIndex)`, so without the shared lock a concurrent pair mints duplicate chunk indices or merges two groups. `FOR UPDATE SKIP LOCKED` only guards the claim/update path, NOT inserts
- `POST /tasks/rename` renames every block of a `(name, date)` group atomically. It locks BOTH name domains (old + new, acquired in sorted order to avoid deadlock), checks the target name is free on that day (else 409), then updates all rows. 404 if the source group has no rows. Uses a `YYYY-MM-DD` string `pattern` (NOT `format: date`) in the spec so Orval keeps it a string instead of coercing to `z.date()`
