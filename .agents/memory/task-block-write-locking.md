...
**How to apply:** When adding/editing any route that inserts into or relocates rows within a task group, wrap it in `db.transaction` and take `pg_advisory_xact_lock(hashtext(name))` first (see `complete-focus`, `add-block`, the note-create path, `POST /tasks` create, and `POST /tasks/rename` in `artifacts/api-server/src/routes/tasks.ts`). Once serialized under the lock, a plain ordered claim is race-free — no SKIP LOCKED needed.

**Rename is a special case:** `POST /tasks/rename` moves a whole group from one `name` to another, so it touches TWO name domains. It must lock both `oldName` and `newName`, acquired in sorted order (`[name, newName].sort()`) to avoid deadlock with a concurrent rename going the other way, then check the target name is free on that date (else 409) before updating. A single-name lock would not serialize it against creates/appends of the new name.
